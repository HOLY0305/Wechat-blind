...
| 功能 | 说明 | 优先级 | 状态 |
|------|------|--------|------|
| 设置页面 UI 重新设计 | 优化视觉布局、颜色、字体（苹果风格） | 高 | 待开发 |
| 应用图标 | 托盘图标、窗口图标 | 高 | ✓ 已完成 |
| 设置实时预览 | 调整透明度/模糊度时实时显示效果 | 高 | ✓ 已完成 |
| 遮罩自定义图案 | 支持用户上传图片作为遮罩内容 | 中 | 待开发 |
| 预设图案库 | 提供默认图案选项（几何、渐变等） | 中 | 待开发 |
| 鼠标悬停策略配置 | 让用户选择"悬停时是否隐藏" | 低 | 待开发 |
